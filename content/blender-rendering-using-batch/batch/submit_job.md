...
Run the following to generate the python script that will download the blender file from S3, launch a job to perform the rendering in a distributed fashion and upload the rendered files to S3.

```bash
cat <<EoF > ~/job_sumbission.py
...
The console will show the arguments that need to be passed onto the script and what is the purpose of each of them. To submit the rendering job, run the following block of code replacing the values of the arguments -i, -o and -f.

```bash
export JOB_ID=$(python3 job_sumbission.py -i s3://blender-rendering/blendfile.blend -o s3://blender-rendering/renders/ -f 50 -n RenderingJob -q "${RENDERING_QUEUE_NAME}" -d "${JOB_DEFINITION_NAME}" | jq -r '.jobId')
```
